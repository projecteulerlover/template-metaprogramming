...
void variadics_test() {
  ...
  try_and_print_exception(
      [] { variadics::printf("Too many parameters %\n", 42, 43); });
}
...
void policies_test() {
  try_and_print_exception(
      [] { policies::non_null_ptr<std::string> ps{nullptr}; });
}
...
void member_detection_test() {
  ...
  member_detection::dump(std::cout,
...
void solutions_test() {
  std::cout << "Lab 1 tests.\n";
  std::cout << "Correct outputs: \n"
            << solutions::lab1_direct::euclidean_distance(
  ...
  std::cout << "My outputs: \n"
            << solutions::my_lab1_direct::euclidean_distance(
                   std::make_tuple(1, 0, 2), std::make_tuple(1, 2, 3))
            << "\n\n";

  auto tup = std::make_tuple(4.0, "hello", 52);
  solutions::lab2::get<double>(tup) = 3.0;
  std::cout << "Lab 2 tests.\n";
  std::cout << "Correct outputs: \n" << solutions::lab2::get<int>(tup) << "\n";
  std::cout << "My outputs: \n" << solutions::my_lab2::get<int>(tup) << "\n\n";
