...
struct helper {
  ...
  static double sum_squared_deltas(Tup1 const& tup1, Tup2 const& tup2) {
    return pow(std::get<I - 1>(tup1) - std::get<I - 1>(tup2), 2) +
           helper<I - 1>::sum_squared_deltas(tup1, tup2);
  }
};
template <>
struct helper<0> {
...
double euclidean_distance(Tup1 const& tup1, Tup2 const& tup2) {
  static_assert(std::tuple_size<Tup1>::value == std::tuple_size<Tup2>::value,
                "two tuples are not the same size.");
  return pow(
      helper<std::tuple_size<Tup1>::value>::sum_squared_deltas(tup1, tup2), .5);
}
}  // namespace my_lab1_direct
...
auto euclidean_distance(Tup1 const& tup1, Tup2 const& tup2) {
  ...
  double ssd =
      helper<std::tuple_size<Tup1>::value - 1>::sum_squared_deltas(tup1, tup2);
  return std::sqrt(ssd);
}
}  // namespace lab1_direct
...
double sum_squared_deltas(Tup1 const& tup1, Tup2 const& tup2,
                          std::index_sequence<Ix...>) {
  double deltas[] = {
      (static_cast<double>(std::get<Ix>(tup1) - std::get<Ix>(tup2)))...};
  return std::accumulate(
      std::begin(deltas), std::end(deltas), 0.0,
...
T& get(std::tuple<Ts...>& ts) {
  static_assert(count<T, Ts...>::value == 1);
  return std::get<find<T, Ts...>::value>(ts);
}
}  // namespace my_lab2
...
constexpr auto has_eq_operator(int)
    -> decltype(std::declval<T>() == std::declval<S>(), true) {
  return true;
}
...
It linear_search(It first, It last, T const& val) {
  using inner_value_t = std::remove_cvref<decltype(*first)>;
  constexpr bool ok = has_eq_operator<T, inner_value_t>(0);
  static_assert(ok,
                "'val' must have an equality operator with elements of the "
                "sequence (==)");
...
struct is_container_better<std::basic_string<CharT, Traits, Allocator>>
    : false_t {};
...
class window {
 ...
 public:
  template <typename Toolbar, typename = std::enable_if_t<!std::is_same<
                                  window, std::decay_t<Toolbar>>::value>>
  window(Toolbar&& toolbar) : toolbar_{std::forward<Toolbar>(toolbar)} {}
  window(window const& other) { /* some copy construction */
  }
  window(window&& other) { /* some move construction */
  }
